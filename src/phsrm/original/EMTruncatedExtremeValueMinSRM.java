...
public class EMTruncatedExtremeValueMinSRM extends EMCommonSRM {

	TruncatedExtremeValueMinSRM srm;
	double omega;
	double shape;
	double scale;
	int dsize;
	...
	public EMTruncatedExtremeValueMinSRM(TruncatedExtremeValueMinSRM srm) {
		this.srm = srm;
	}
	...
	public double doEMstep() {
		...
		llf = 0.0;
		g00 = Math.exp(-y0);
		g01 = Math.exp(-scale/shape)*((1.0 + y0)*Math.exp(-y0));
		g02 = Math.exp(-y0) * (1.0 + y0 * Math.log(y0));
		t = fdat.getTime(1);
		x = fdat.getNumber(1);
		y = Math.exp((t+scale)/shape);
		g10 = Math.exp(-y);
		g11 = Math.exp(-scale/shape)*((1.0 + y)*Math.exp(-y));
		g12 = Math.exp(-y) * (1.0 + y * Math.log(y));
		if (x != 0.0) {
			tmp1 = g00 - g10;
			tmp2 = g01 - g11;
			tmp3 = g02 - g12;
		...
		for (int j=2; j<=fdat.getNumberOfRecords(); j++) {
			...
			y = Math.exp((t+scale)/shape);
			g20 = Math.exp(-y);
			g21 = Math.exp(-scale/shape)*((1.0 + y)*Math.exp(-y));
			g22 = Math.exp(-y) * (1.0 + y * Math.log(y));
			if (x != 0.0) {
				tmp1 = g10 - g20;
				tmp2 = g11 - g21;
				tmp3 = g12 - g22;
