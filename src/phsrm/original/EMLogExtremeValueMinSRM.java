...
public class EMLogExtremeValueMinSRM extends EMCommonSRM {

	LogExtremeValueMinSRM srm;
	double omega;
	double shape;
	double scale;
	int dsize;
	...
	public EMLogExtremeValueMinSRM(LogExtremeValueMinSRM srm) {
		this.srm = srm;
	}
	...
	public double doEMstep() {
		...
		llf = 0.0;
		g00 = Math.exp(-y);
		g01 = Math.exp(-scale/shape)*(1.0 + y)*Math.exp(-y);
		g02 = Math.exp(-y) * (1.0 + y * Math.log(y));
		if (x != 0.0) {
			tmp1 = 1.0 - g00;
