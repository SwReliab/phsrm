...
public class EMTruncatedLogisticSRM extends EMCommonSRM {

	TruncatedLogisticSRM srm;
	double omega;
	double shape;
	double scale;
	int dsize;
	...
	public EMTruncatedLogisticSRM(TruncatedLogisticSRM srm) {
		this.srm = srm;
	}
	...
	public double doEMstep() {
		...
		g02 = (1.0+(1.0+Math.log(y0))*y0)/((1.0+y0)*(1.0+y0));
		t = fdat.getTime(1);
		x = fdat.getNumber(1);
		...
		for (int j=2; j<=fdat.getNumberOfRecords(); j++) {
			...
			g22 = (1.0+(1.0+Math.log(y))*y)/((1.0+y)*(1.0+y));
			if (x != 0.0) {
				tmp1 = g10 - g20;
				tmp2 = g11 - g21;
				tmp3 = g12 - g22;
		...
		en2 += omega * (0.5 - g01 + g11);  // g01 is the first, g11 is the last
